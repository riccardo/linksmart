 ...
 *
 */
@Component(name="IdentityManagerCrypto", immediate=true)
@Service({IdentityManager.class})
public class IdentityManagerCertImpl implements IdentityManager, MessageProcessor {
	...
	public IdentityManagerCertImpl() {
		init();
	}
	...
	protected void bindCryptoManager(CryptoManager cryptoManager) {
		LOG.debug("IdentityManagerCert::binding cryptomanager");
		this.cryptoManager = cryptoManager;	
	}

	protected void unbindCryptoManager(CryptoManager cryptoManager) {
		LOG.debug("IdentityManagerCert::un-binding cryptomanager");
	...
	public Registration createServiceByAttributes(Part[] parts) {
		...
		try{
			...
			LOG.debug("Created VirtualAddress (CERT): " + info.toString());
			return info;
		}catch(Exception e){
