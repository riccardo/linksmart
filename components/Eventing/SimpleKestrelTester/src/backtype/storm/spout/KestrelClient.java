...
//    }
     ...
     * @return
     */
    private byte[] deleteCommand(String queueName) {
    	return ("DELETE " + queueName + "\r\n").getBytes();
	}

	private byte[] flushCommand(String queueName) {
		return ("FLUSH " + queueName + "\r\n").getBytes();
	}
    ...
    private Item parseDequeueOutputItem(String[] header_tokens) 
        ...
        byte[] data = 
        		_mis.readFixedLength(num_bytes);
//        		_mis.readLine().getBytes();
    ...
    public boolean flush(String queueName) throws IOException, ParseError{
    	_os.write(flushCommand(queueName));
        return true;
    }
    
    public boolean delete(String queueName) throws IOException, ParseError{
    	_os.write(deleteCommand(queueName));
        return true;
    }
}
