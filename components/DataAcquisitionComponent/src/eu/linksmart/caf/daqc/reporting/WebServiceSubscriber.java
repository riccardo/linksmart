...
/**
 * Implemented {@link SubscriberStub} for communicating with a
 * {@link DaqcReportingService} over Web Services
 * 
 * @author Michael Crouch
 */
public class WebServiceSubscriber extends SubscriberStub {

	/** the {@link Logger} */
	private static final Logger logger = Logger.getLogger(WebServiceSubscriber.class);
	 ...
	 *            the {@link Subscriber}
	 */
	public WebServiceSubscriber(DAqCApplication daqc, Subscriber subscriber) {
		super(subscriber);
		this.daqc = daqc;
	}
		...
		try {
			...
			String targetUrl =
					"http://localhost:" + portStr + "/SOAPTunneling/"
							+ daqc.getHid() + "/"
							+ this.getSubscriber().getSubscriberHid() + "/" + 0
							+ "/hola";
			Service service = new Service();
			Call call = (Call) service.createCall();
			call.setTargetEndpointAddress(new URL(targetUrl));
			call.setOperationName(new QName("http://daqc.caf.linksmart.eu",
					"reportAcquiredData"));

			return (Boolean) call.invoke(new Object[] { report });
		} catch (Exception e) {
			logger.error("Error reporting data (over WS) to "
					+ this.getSubscriber().getSubscriberHid(), e);
			return false;
		}
	}
