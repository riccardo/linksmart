...
public class LockManagerServlet extends HttpServlet {
	...
	public LockManagerServlet(String serverHost, int port, String path){
		super();
		try {
		...
		LockManagerEndPoint.createEndPoints(serverHost, port);
			...
			try {
        		...
        		requestAsString = requestAsString.concat(request.getMethod()+" / "+request.getProtocol()+"\r\n");
           		...
           		while( headerNames.hasMoreElements()){
            		...
            		for(int i=0; i<LockManagerEndPoint.getEndPoints().size(); i++){
				responseAsString = ((LockManagerEndPoint)LockManagerEndPoint.getEndPoints().elementAt(i)).handleRequest(requestAsString,this.clientHost);
					if(responseAsString != null)
						i = LockManagerEndPoint.getEndPoints().size();
	    		}
        	}
            	catch(Exception e)
            	{
