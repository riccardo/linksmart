...
//import eu.linksmart.limbo.storagemanagerdevice.StringTokenizer;

public class LockManagerParser {
	...
	public LockManagerParser(String total, String soapaction){
		this.request = total;
	...
	public Vector getLockInfo(String subRequest){
		...
		if((subRequest.indexOf("<lock",0)!=-1)) {
...
public Vector getLockTypes(String subRequest){
		...
		before ="getLockTypes";
		after = "getLockTypes>";
		if((subRequest.indexOf("getLockTypes")!=-1) && (subRequest.endsWith("/>"))) {
			result = "";
			this.nrArgs= nArgs;
			return V;
		}
