...
package eu.linksmart.limbo.lockmanager.handler;

import eu.linksmart.limbo.lockmanager.*;

public class LockManagerSOAPHandler extends LockManagerHandler{
	
	public LockManagerSOAPHandler(LockManagerHandlers context) {
		super(context);
	}
	...
	public String handleRequest(String theRequest) {
		...
		LockManagerOpsImpl operations = super.getContext().getService().getOperations();
		...
		if(super.getContext().getService().hasOperation(theSOAPAction)){
			LockManagerParser p = new LockManagerParser(theSOAPRequest, theSOAPAction);
			if(p.parseRequest(theSOAPRequest)){
			if(p.getOperation().equalsIgnoreCase("getLockInfo") && theSOAPAction.equalsIgnoreCase("getLockInfo")){
				String s = "";
				...
				s = s.concat("<soapenv:Body><getLockInfoResponse xmlns="+'"'+'"'+"><result>"+operations.getLockInfo(p.getArg(0).toString())+"</result></getLockInfoResponse></soapenv:Body></soapenv:Envelope>");
			...
			if(p.getOperation().equalsIgnoreCase("getLock") && theSOAPAction.equalsIgnoreCase("getLock")){
				String s = "";
				s = s.concat("<?xml version="+'"'+"1.0"+'"'+" encoding="+'"'+"utf-8"+'"'+"?>");
				s = s.concat("<soapenv:Envelope xmlns:soapenv="+'"'+"http://schemas.xmlsoap.org/soap/envelope/"+'"'+" xmlns:xsd="+'"'+"http://www.w3.org/2001/XMLSchema"+'"'+" xmlns:xsi="+'"'+"http://www.w3.org/2001/XMLSchema-instance"+'"'+">");
				s = s.concat("<soapenv:Body><getLockResponse xmlns="+'"'+'"'+"><result>"+operations.getLock(p.getArg(0).toString())+"</result></getLockResponse></soapenv:Body></soapenv:Envelope>");
				String result = "HTTP/1.1 200 OK\r\n";
				result = result.concat("Server: Limbo Server 1.0 (LinkSmart x86 java/1.5.0_06\r\n");
				result = result.concat("Content-Type: text/xml; charset=utf-8\r\nConnection: close\r\n");
				result = result.concat("Content-Length: "+s.getBytes().length+"\r\n\r\n");
			...
			if(p.getOperation().equalsIgnoreCase("getLockTypes") && theSOAPAction.equalsIgnoreCase("getLockTypes")){
					String s = "";
					s = s.concat("<?xml version="+'"'+"1.0"+'"'+" encoding="+'"'+"utf-8"+'"'+"?>");
					s = s.concat("<soapenv:Envelope xmlns:soapenv="+'"'+"http://schemas.xmlsoap.org/soap/envelope/"+'"'+" xmlns:xsd="+'"'+"http://www.w3.org/2001/XMLSchema"+'"'+" xmlns:xsi="+'"'+"http://www.w3.org/2001/XMLSchema-instance"+'"'+">");
					s = s.concat("<soapenv:Body><getLockTypesResponse xmlns="+'"'+'"'+"><result>"+operations.getLockTypes()+"</result></getLockTypesResponse></soapenv:Body></soapenv:Envelope>");
					String result = "HTTP/1.1 200 OK\r\n";
					result = result.concat("Server: Limbo Server 1.0 (LinkSmart x86 java/1.5.0_06\r\n");
					result = result.concat("Content-Type: text/xml; charset=utf-8\r\nConnection: close\r\n");
					result = result.concat("Content-Length: "+s.getBytes().length+"\r\n\r\n");
