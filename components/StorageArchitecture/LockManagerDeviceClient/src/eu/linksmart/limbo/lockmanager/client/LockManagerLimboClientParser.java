...
public class LockManagerLimboClientParser {
	...
	public LockManagerLimboClientParser(String total, String soapaction){
		this.request = total;
	...
	public boolean parseRequest(String request){
		...
		if(result == null)
			return false;
		Vector op;
		op = this.getLockInfo(result);
		this.operation = "getLockInfo";
		if(op == null){
		op = this.getLock(result);
		this.operation = "getLock";
		if(op == null){
		op = this.getLockTypes(result);
		this.operation = "getLockTypes";
		}
		if(op == null){
		return false;
		}
	...
	public String preamble(String subRequest){
		...
		//System.out.println("in preamble: "+subRequest.regionMatches(true, 0, preamble, 0, preamble.length()));
		...
		//System.out.println("in preamble result= "+ result);
		}else
			result = null;
		return result;
	}
	...
	public Vector getLockInfo(String subRequest){
		...
		nArgs = 1;
		V = new Vector(nArgs);
		before ="getLockInfoResponse";
		after = "getLockInfoResponse>";
...
public Vector getLock(String subRequest){
		...
		nArgs = 1;
		V = new Vector(nArgs);
		before ="getLockResponse";
		after = "getLockResponse>";
		if((subRequest.indexOf(before)!=-1) && (subRequest.endsWith(after))) {
			st = new StringTokenizer(subRequest,">");
			before = st.nextToken();
			before = before.concat(">");
...
public Vector getLockTypes(String subRequest){
		...
		nArgs = 1;
		V = new Vector(nArgs);
		before ="getLockTypesResponse";
		after = "getLockTypesResponse>";
		if((subRequest.indexOf(before)!=-1) && (subRequest.endsWith(after))) {
			st = new StringTokenizer(subRequest,">");
			before = st.nextToken();
			before = before.concat(">");
