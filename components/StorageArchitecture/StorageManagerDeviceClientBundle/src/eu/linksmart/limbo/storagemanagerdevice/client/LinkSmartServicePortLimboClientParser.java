...
public class LinkSmartServicePortLimboClientParser {
	...
	public LinkSmartServicePortLimboClientParser(String total, String soapaction){
		this.request = total;
	...
	public Vector CreateWS(String subRequest){
		...
		nArgs = 1;
		V = new Vector(nArgs);
		before ="createWSResponse";
		after = "createWSResponse>";
		if((subRequest.indexOf(before)!=-1) && (subRequest.endsWith(after))) {
			st = new StringTokenizer(subRequest,">");
			before = st.nextToken();
			before = before.concat(">");
	...
	public Vector GetDiscoveryInfo(String subRequest){
		...
		nArgs = 1;
		V = new Vector(nArgs);
		before ="getDiscoveryInfoResponse";
		after = "getDiscoveryInfoResponse>";
		if((subRequest.indexOf(before)!=-1) && (subRequest.endsWith(after))) {
			st = new StringTokenizer(subRequest,">");
			before = st.nextToken();
			before = before.concat(">");
			result = subRequest.substring(before.length(), subRequest.lastIndexOf('<'));
			before = "<DiscoveryInfo";
			after = "</DiscoveryInfo>";
			result = result.substring(before.length());
			index = result.indexOf('>');
			result = result.substring(index+1);
	...
	public Vector GetWSEndpoint(String subRequest){
		...
		nArgs = 1;
		V = new Vector(nArgs);
		before ="getWSEndpointResponse";
		after = "getWSEndpointResponse>";
		if((subRequest.indexOf(before)!=-1) && (subRequest.endsWith(after))) {
			st = new StringTokenizer(subRequest,">");
			before = st.nextToken();
			before = before.concat(">");
	...
	public Vector GetLinkSmartWSEndpoint(String subRequest){
		...
		nArgs = 1;
		V = new Vector(nArgs);
		before ="getLinkSmartWSEndpointResponse";
		after = "getLinkSmartWSEndpointResponse>";
		if((subRequest.indexOf(before)!=-1) && (subRequest.endsWith(after))) {
			st = new StringTokenizer(subRequest,">");
			before = st.nextToken();
			before = before.concat(">");
			result = subRequest.substring(before.length(), subRequest.lastIndexOf('<'));
			before = "<LinkSmartWSEndpoint";
			after = "</LinkSmartWSEndpoint>";
			result = result.substring(before.length());
			index = result.indexOf('>');
			result = result.substring(index+1);
			index = result.lastIndexOf('<');
	...
	public Vector GetWSDL(String subRequest){
		...
		nArgs = 1;
		V = new Vector(nArgs);
		before ="getWSDLResponse";
		after = "getWSDLResponse>";
		if((subRequest.indexOf(before)!=-1) && (subRequest.endsWith(after))) {
			st = new StringTokenizer(subRequest,">");
			before = st.nextToken();
			before = before.concat(">");
	...
	public Vector Stop(String subRequest){
		...
		if((subRequest.indexOf("stopResponse")!=-1) && (subRequest.endsWith("/>"))) {
			result = "";
			this.nrArgs= nArgs;
			return V;
		}		if((subRequest.indexOf(before)!=-1) && (subRequest.endsWith(after))) {
	...
	public Vector StopWS(String subRequest){
		...
		nArgs = 0;
		V = new Vector(nArgs);
		before ="stopWSResponse";
		after = "stopWSResponse>";

		if((subRequest.indexOf("stopWSResponse")!=-1) && (subRequest.endsWith("/>"))) {
			result = "";
			this.nrArgs= nArgs;
			return V;
		}		if((subRequest.indexOf(before)!=-1) && (subRequest.endsWith(after))) {
	...
	public Vector StopLinkSmartWS(String subRequest){
		...
		nArgs = 0;
		V = new Vector(nArgs);
		before ="stopLinkSmartWSResponse";
		after = "stopLinkSmartWSResponse>";

		if((subRequest.indexOf("stopLinkSmartWSResponse")!=-1) && (subRequest.endsWith("/>"))) {
			result = "";
			this.nrArgs= nArgs;
			return V;
		}		if((subRequest.indexOf(before)!=-1) && (subRequest.endsWith(after))) {
