...
public class CookieParser {
	...
	public CookieParser(String total, String soapaction) {
		this.request = total;
	...
	public Object getArg(int position) {
		return this.args.elementAt(position);
	}
	...
	public String preamble(String subRequest) {
		String result;
		String preamble = "<?xml version=" + '"' + "1.0" + '"' + " encoding="
				+ '"' + "utf-8" + '"' + "?>";
		if (subRequest.regionMatches(true, 0, preamble, 0, preamble.length()))
			result = subRequest.substring(preamble.length(),
					subRequest.length());
		else
			result = null;
		return result;
	...
	public java.util.Vector getMultiRefIDs(String subRequest) {
		...
		while (hasMoreIds) {
			int beforeIndex = subRequest.indexOf("href=\"#")
					+ "href=\"#".length();
			String after = subRequest.substring(beforeIndex,
					subRequest.length());
			int afterIndex = after.indexOf("\"") - 1;
			String id = subRequest.substring(beforeIndex,
					afterIndex + "\"".length() + beforeIndex);
			ids.add(id);
			subRequest = subRequest.substring(afterIndex + "\"".length()
					+ beforeIndex);
	...
	public String replaceRefs(Hashtable multiRefs, String request) {
		...
		int multiRefEnd = request.lastIndexOf("</multiRef>")
				+ "</multiRef>".length();
		...
		while (hasMoreIds) {
			...
			int afterIndex = after.indexOf("\"") - 1;
			String id = request.substring(beforeIndex,
					afterIndex + "\"".length() + beforeIndex);
			String before = request.substring(0, beforeIndex);
			String tagElement = before.substring(before.lastIndexOf("<") + 1,
					before.length() - "href=\"#".length() - 1);
			int beginTag = before.lastIndexOf("<");
			int endTag = request.indexOf(">", beginTag);
			String refData = multiRefs.get(id).toString();
			request = request.replace(request.substring(beginTag, endTag), "<"
					+ tagElement + ">" + refData + "</" + tagElement);
	...
	public String soapenv(String subRequest) {
		...
		if (st2.hasMoreTokens()) {
			...
			if (subRequest.startsWith(before) && subRequest.endsWith(after)
					&& (env.equalsIgnoreCase("Envelope"))) {
				StringTokenizer st = new StringTokenizer(subRequest, ">");
				before = st.nextToken();
				before = before.concat(">");
				result = subRequest.substring(before.length(),
						(subRequest.length() - after.length()));
			} else
				result = null;
		} else
			result = null;
	...
	public String soapbody(String subRequest) {
		...
		if (st.hasMoreTokens()) {
			...
			if (subRequest.startsWith(before) && subRequest.endsWith(after)
					&& (body.equalsIgnoreCase("Body"))) {
				int firstIndex = subRequest.indexOf(">");
				result = subRequest.substring(firstIndex + 1,
						(subRequest.length() - after.length()));
			} else
				result = null;
		} else
	...
	public Vector getCookieNames(String subRequest) {
		...
		nArgs = 0;
		V = new Vector(nArgs);
		before = "getCookieNames";
		after = "getCookieNames>";

		if ((subRequest.indexOf("getCookieNames") != -1)
				&& (subRequest.endsWith("/>"))) {
			result = "";
			this.nrArgs = nArgs;
			return V;
	...
	public Vector getAllData(String subRequest) {
		...
		after = "getAllData>";
		if ((subRequest.indexOf(before) != -1) && (subRequest.endsWith(after))) {
			if ((subRequest.indexOf("<regularExpression", 0) != -1)) {
				st = new StringTokenizer(subRequest, ">");
				before = st.nextToken();
				before = before.concat(">");
				result = subRequest.substring(before.length(),
	...
	public Vector getCookieValue(String subRequest) {
		...
		nArgs = 1;
		V = new Vector(nArgs);
		before = "getCookieValue";
		after = "getCookieValue>";
		if ((subRequest.indexOf(before) != -1) && (subRequest.endsWith(after))) {
			if ((subRequest.indexOf("<cookieName", 0) != -1)) {
				st = new StringTokenizer(subRequest, ">");
				before = st.nextToken();
				before = before.concat(">");
				result = subRequest.substring(before.length(),
	...
	public Vector setCookieValue(String subRequest) {
		...
		nArgs = 2;
		V = new Vector(nArgs);
		before = "setCookieValue";
		after = "setCookieValue>";
		if ((subRequest.indexOf(before) != -1) && (subRequest.endsWith(after))) {
			if ((subRequest.indexOf("<cookieName", 0) != -1)
					&& (subRequest.indexOf("<cookieValue", 0) != -1)) {
				st = new StringTokenizer(subRequest, ">");
				before = st.nextToken();
				before = before.concat(">");
				result = subRequest.substring(before.length(),
