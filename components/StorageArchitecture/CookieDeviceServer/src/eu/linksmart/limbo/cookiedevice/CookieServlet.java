...
public class CookieServlet extends HttpServlet {
	...
	public CookieServlet(String serverHost, int port, String path) {
		super();
		try {
		...
		CookieEndPoint.createEndPoints(serverHost, port);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (int i = 0; i < CookieEndPoint.getEndPoints().size(); i++) {
				responseAsString = ((CookieEndPoint) CookieEndPoint
						.getEndPoints().elementAt(i)).handleRequest(
						requestAsString, this.clientHost);
				if (responseAsString != null)
					i = CookieEndPoint.getEndPoints().size();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
