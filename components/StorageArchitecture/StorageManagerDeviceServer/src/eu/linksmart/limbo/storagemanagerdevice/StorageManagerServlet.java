...
public class StorageManagerServlet extends HttpServlet {
	...
	public StorageManagerServlet(String serverHost, int port, String path){
		super();
		try {
		...
		StorageManagerEndPoint.createEndPoints(serverHost, port);
			...
			try {
        		...
        		requestAsString = requestAsString.concat(request.getMethod()+" / "+request.getProtocol()+"\r\n");
           		...
           		while( headerNames.hasMoreElements()){
            		...
            		for(int i=0; i<StorageManagerEndPoint.getEndPoints().size(); i++){
				responseAsString = ((StorageManagerEndPoint)StorageManagerEndPoint.getEndPoints().elementAt(i)).handleRequest(requestAsString,this.clientHost);
					if(responseAsString != null)
						i = StorageManagerEndPoint.getEndPoints().size();
	    		}
        	}
            	catch(Exception e)
            	{
