...
import eu.linksmart.limbo.storagemanagerdevice.StorageManagerDeviceActivator;

public class StorageManagerDeviceDevice implements UPnPDevice,UPnPEventListener,ServiceListener  {

	final private String DEVICE_ID = "uuid:StorageManagerDeviceDevice+" +Integer.toHexString(new Random(System.currentTimeMillis()).nextInt());
	private final static String STORAGEMANAGERDEVICE_DEVICE_TYPE = "urn:schemas-upnp-org:linksmartdevice:Laptop_1:1";
	private final static String STORAGEMANAGER_TYPE = "urn:schemas-upnp-org:service:StorageManager:1";
	...
	private StorageManagerUPnPService StorageManager;
	private UPnPEventNotifier StorageManagerNotifier;
	...
	private configStateVariable configState;
	private idStateVariable idState;
	...
	private StorageManagerDeviceActivator deviceActivator;
	
	public StorageManagerDeviceDevice(BundleContext context) {
		this.context = context;
		StorageManager = new StorageManagerUPnPService(this, context);
		LinkSmartServicePort = new LinkSmartServicePortUPnPService(this);
		services = new UPnPService[]{StorageManager,LinkSmartServicePort};		
		resultState = (resultStateVariable) StorageManager.getStateVariable("result");
		configState = (configStateVariable) StorageManager.getStateVariable("config");
		idState = (idStateVariable) StorageManager.getStateVariable("id");
		...
		buildEventNotifyer();
		try {
			StorageManagerDeviceUPnPActivator.context.addServiceListener(this,devicesFilter);
	...
	private void buildEventNotifyer() {
		StorageManagerNotifier = new UPnPEventNotifier(StorageManagerDeviceUPnPActivator.context, this, StorageManager);
		resultState.setNotifier(StorageManagerNotifier);
		configState.setNotifier(StorageManagerNotifier);
		idState.setNotifier(StorageManagerNotifier);
		LinkSmartServicePortNotifier = new UPnPEventNotifier(StorageManagerDeviceUPnPActivator.context, this, LinkSmartServicePort);
	...
	private void setupDeviceProperties(){
		...
		dictionary.put(UPnPDevice.FRIENDLY_NAME,"StorageManager");
		dictionary.put(UPnPDevice.MANUFACTURER,"UoP");
		dictionary.put(UPnPDevice.MANUFACTURER_URL,"www.linksmart.eu");
		dictionary.put(UPnPDevice.MODEL_DESCRIPTION,"Storage Manager device supporting File System Devices");
		dictionary.put(UPnPDevice.MODEL_NAME,"2");
		dictionary.put(UPnPDevice.MODEL_NUMBER,"2-A");
		dictionary.put(UPnPDevice.TYPE,STORAGEMANAGERDEVICE_DEVICE_TYPE);
	...
	public UPnPService getService(String serviceId) {
		if  (serviceId.equals(StorageManager.getId())) return StorageManager;
	...
	public void doServiceRegistration() {
		...
		try {
			...
			this.deviceActivator = new StorageManagerDeviceActivator(this);
