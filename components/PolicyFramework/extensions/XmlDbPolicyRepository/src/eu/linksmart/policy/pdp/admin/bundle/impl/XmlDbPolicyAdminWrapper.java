...
/**
 * <p>Bundle wrapper for {@link XMLDBPdpAdminService}</p>
 * 
 * @author Marco Tiemann
 *
 */
public class XmlDbPolicyAdminWrapper implements PdpAdmin {

	/** logger */
	private static final Logger logger 
			= Logger.getLogger(XmlDbPolicyAdminWrapper.class);	
	...
	private XMLDBPdpAdminService admin = null;
	
	/** {@link XmlDbPolicyAdminConfigurator */
	private XmlDbPolicyAdminConfigurator configurator = null;
	...
	public void applyConfigurations(Hashtable<?, ?> theUpdates) {
		if (theUpdates.contains(XmlDbPolicyAdminConfigurator.DB_PATH)) {
			try {
	...
	/**
	 * @return
	 * 				the {@link XMLDBPdpAdminService} or <code>null</code>
	 */
	public XMLDBPdpAdminService getAdmin() {
		return admin;
	}
	...
	protected void activate(ComponentContext theContext) {
		...
		configurator = new XmlDbPolicyAdminConfigurator(context, this);
		configurator.init();
		configurator.registerConfiguration();
		Dictionary conf = configurator.getConfiguration();
		Enumeration enu = conf.keys();
		...
		try {
			logger.debug("Creating XMLDBPdpAdminService");
			admin = new XMLDBPdpAdminService(
						configurator.get(XmlDbPolicyAdminConfigurator.DB_PATH),
						context);
		} catch (Exception e) {
