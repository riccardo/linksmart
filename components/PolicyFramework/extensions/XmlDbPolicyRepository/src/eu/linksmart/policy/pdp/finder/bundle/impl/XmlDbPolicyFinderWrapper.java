...
/**
 * <p>Bundle wrapper for {@link PolicyDbFinderModule}</p>
 * 
 * @author Marco Tiemann
 *
 */
public class XmlDbPolicyFinderWrapper extends PolicyFinderModule {

	/** logger */
	static final Logger logger 
			= Logger.getLogger(XmlDbPolicyFinderWrapper.class);	
	...
	/** {@link PolicyDbFinderModule} */
	private PolicyDbFinderModule finder = null;
	
	/** {@link XmlDbPolicyFinderConfigurator} */
	private XmlDbPolicyFinderConfigurator configurator = null;
	...
	protected void activate(ComponentContext theContext) {
		...
		configurator = new XmlDbPolicyFinderConfigurator(
	...
	void register(ServiceReference theService) {
		...
		if (module instanceof XmlDbPolicyAdminWrapper) {
			logger.debug("Found XmlDbPolicyAdminWrapper, registering");
			logger.debug("Setting PdpAdmin "
					+ module.getClass().getCanonicalName() + ", service name "
					+ theService.getBundle().getSymbolicName());
			finder = new PolicyDbFinderModule(
					((XmlDbPolicyAdminWrapper) module).getAdmin());
			logger.debug("Instantiated finder: " 
					+ finder.getClass().getCanonicalName());
		} else {
