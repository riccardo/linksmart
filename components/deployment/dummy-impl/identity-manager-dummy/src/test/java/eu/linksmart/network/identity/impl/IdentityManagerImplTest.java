...
public class IdentityManagerImplTest {
	...
	private IdentityManagerImplDummy identityMgr;

	@Before
	public void setUp() throws RemoteException {
		//create IdMgr and store entities 
		this.identityMgr = new IdentityManagerImplDummy();
...
//		when(this.identityMgr.networkManagerCore.broadcastMessage(any(Message.class))).
...
//			when(this.identityMgr.networkManagerCore.getService()).thenReturn(NM_VIRTUAL_ADDRESS);
	...
	public void getServiceByAttributesTestTimeOut() {
		...
		this.identityMgr.getServiceByAttributes(new Part[]{ATTR_1},	TIMEOUT, false, false);
//		assertTrue(
...
//				Calendar.getInstance().getTimeInMillis() <= startTime + TIMEOUT + TOLERANCE);
	...
	public void getServiceByAttributesTestReturnFirst() {
		long startTime = Calendar.getInstance().getTimeInMillis();
		this.identityMgr.getServiceByAttributes(new Part[]{ATTR_1},	TIMEOUT, true, false);
		assertTrue(
