...
namespace OpenSource.UPnP
{
    ...
    /// </summary>
    public class UPnPInternalSmartControlPoint //Removed sealed and internal, added pubvlic Peeter Kool Hydra project
    {
        ...
        private void UPnPControlPointSearchSink(IPEndPoint source, IPEndPoint local, Uri LocationURL, String USN, String SearchTarget, int MaxAge)
        {
            ...
            lock (deviceTableLock)
            {
                if (deviceTable.ContainsKey(USN) == false)
                {
                    ...
                    deviceFactory.CreateDevice(deviceInfo.BaseURL, deviceInfo.MaxAge, local.Address);
                }
                else
                {
            ...
            else
            {
                lock (deviceTableLock)
                {
                    ...
                    if (deviceTable.ContainsKey(USN) == false)
                    {
                        ...
                        deviceInfo.SourceEP = source;
                        deviceTable[USN] = deviceInfo;
                        deviceFactory.CreateDevice(deviceInfo.BaseURL, deviceInfo.MaxAge, local.Address); // TODO: Does URI construction work all this time??
                    }
                    else
                    {
        ...
        private void DeviceFactoryCreationSink(UPnPDeviceFactory sender, UPnPDevice device, Uri locationURL)
        {
            ...
            lock (deviceTableLock)
            {
                ...
                {
                    OpenSource.Utilities.EventLogger.Log(this, System.Diagnostics.EventLogEntryType.Information,
                        "Unexpected UPnP Device Creation: " + device.FriendlyName + "@" + device.LocationURL);
                    return;
                }
                // Lets update out state and notify the user.
                DeviceInfo deviceInfo = (DeviceInfo)deviceTable[device.UniqueDeviceName];
