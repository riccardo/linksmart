 ...
 * 
 */
public class EventManagerPortBindingImpl implements
		eu.linksmart.eventmanager.EventManagerPort {
	public static LocalSubscriptions subscriptions = new LocalSubscriptions();
	private static Logger log = Logger
			.getLogger(EventManagerPortBindingImpl.class);

	private static String OWN_IP_ADDRESS  = "129.26.241.128";
	...
	/**
	 * <b>start</b> Starts the EventManagerServer
	 * 
	 * @throws Exception
	 */
	...
	protected void activate(ComponentContext context) {
		...
		configurator = new EventManagerConfigurator(context.getBundleContext(),
				this);
//		try {
//			OWN_IP_ADDRESS = InetAddress.getLocalHost().getHostAddress();
...
//		}
		...
		if (http != null) {
			try {
				log.info("Activating");
				http.registerServlet("/EventManagerStatus",
						new EventManagerServerStatus(context), null, null);
				http.registerServlet("/GetEventManagerSubscriptions",
						new GetEventManagerSubscriptions(context), null, null);
	...
	/*
	 * This method is called: - init() -> Takes the configuration stored -
	 * updateConfiguration - bindWSProvider - bindNetworkManager
	 */
	private void createHIDForEventManager(boolean renewCert) {

		if (eventManagerHID != null)
			return; // Only do this once

		boolean withNetworkManager = Boolean.parseBoolean(configurator
				.get(EventManagerConfigurator.USE_NETWORK_MANAGER));

		log.info("withNetworkManager: " + withNetworkManager);

		if (withNetworkManager) {
			boolean osgi = Boolean.parseBoolean(configurator
					.get(EventManagerConfigurator.USE_NETWORK_MANAGER_OSGI));
			if (osgi == true) {
				// Communicate directly with the Network Manager using OSGi
				if (nmOSGi != null) {
					try {
						if ((configurator
								.get(EventManagerConfigurator.CERT_REF) == null)
								|| renewCert == true) {
							String pid = configurator
									.get(EventManagerConfigurator.PID);
							if ((pid == null) || (pid.equals(""))) {
								pid = "EventManager:"
										+ InetAddress.getLocalHost()
												.getHostName();
							}
							String xmlAttributes = getXMLAttributeProperties(
									pid, "EventManagerPort", pid);
							CryptoHIDResult result = nmOSGi
									.createCryptoHID(
											xmlAttributes,
											"http://"
													+ OWN_IP_ADDRESS
							...
							configurator.setConfiguration(
									EventManagerConfigurator.CERT_REF,
									result.getCertRef());

						} else {
							// TODO refactoring
							// We think this line is not sense making
							// this.eventManagerHID =
							// nmOSGi.createCryptoHIDfromReference(configurator.get(EventManagerConfigurator.CERT_REF),
							// "http://localhost:"+System.getProperty("org.osgi.service.http.port")+
							// EVENT_MANAGER_PATH);
				...
				if (clientProvider != null) {
					if (configurator.get(EventManagerConfigurator.NM_ADDRESS) != null) {
						try {
									...
									this.nmWSClient = (NetworkManagerApplication) clientProvider
											.getRemoteWSClient(
													NetworkManagerApplication.class
															.getName(),
													(String) configurator
															.get(EventManagerConfigurator.NM_ADDRESS),
													false);
								} catch (Exception e1) {
							...
							if (configurator
									.get(EventManagerConfigurator.CERT_REF) != null) {
								this.eventManagerHID = nmWSClient
										.createCryptoHIDfromReference(
												configurator
														.get(EventManagerConfigurator.CERT_REF),
												"http://"
														+ OWN_IP_ADDRESS
														+ ":"
														+ System.getProperty("org.osgi.service.http.port")
								...
								if (this.eventManagerHID == null) {
									...
									String xmlAttributes = getXMLAttributeProperties(
											pid, "EventManagerPort", pid);
									CryptoHIDResult result = nmWSClient
											.createCryptoHID(
													xmlAttributes,
													"http://localhost:"
															+ System.getProperty("org.osgi.service.http.port")
	...
	public String getXMLAttributeProperties(String pid, String sid, String desc)
			throws IOException {
	...
	protected void setClientProvider(RemoteWSClientProvider clientProvider) {
		...
		if (activated) {
			try {
				this.nmWSClient = (NetworkManagerApplication) clientProvider
						.getRemoteWSClient(NetworkManagerApplication.class
								.getName(), (String) configurator
								.get(EventManagerConfigurator.NM_ADDRESS),
								false);
			} catch (Exception e1) {
				log.error(e1.getMessage(), e1);
	...
	public void applyConfigurations(Hashtable updates) {
		// TODO Fill it to change configuration, for example when the
		// withNetworkManager property changes
	...
	public boolean subscribeWithHID(String topic, String hid)
			throws java.rmi.RemoteException {
		try {
			if (!Boolean.parseBoolean(configurator
					.get(EventManagerConfigurator.USE_NETWORK_MANAGER)))
				return false;
			if (eventManagerHID == null)
				return false;
			log.info("EventManager subscribeWithHID on: " + topic + " from "
					+ hid);

			// FIXME Put in the configuration just the address and then build
			// the addresses based on the path, which is always the same
			String nmAddress = ((String) configurator
					.get(EventManagerConfigurator.NM_ADDRESS));
			String nmLocation = nmAddress.substring(0, nmAddress
					.indexOf("/axis/services/NetworkManagerApplication"));
			subscriptions.add(topic, new URL(nmLocation + "/SOAPTunneling"
					+ "/" + eventManagerHID + "/" + hid + "/0/"), hid);
			if (!this.hasTopic(topic)) {
				this.insertTopic(new Topic(topic, PRIORITY));
			}
			return true;
	...
	@Override
	public boolean unsubscribeWithHID(java.lang.String topic,
			java.lang.String hid) throws java.rmi.RemoteException {
		try {
			if (!Boolean.parseBoolean(configurator
					.get(EventManagerConfigurator.USE_NETWORK_MANAGER)))
				return false;
	...
	public eu.linksmart.eventmanager.Subscription[] getSubscriptions()
			throws java.rmi.RemoteException {
		eu.linksmart.eventmanager.Subscription[] result = new eu.linksmart.eventmanager.Subscription[subscriptions
		...
		for (int i = 0; i < subscriptions.getSubscriptions().size(); i++) {
			...
			axisSubscription.setTopic(subscriptions.getSubscriptions().get(i)
					.getTopic());
			if (subscriptions.getSubscriptions().get(i).getHID() != null)
				axisSubscription.setHID(subscriptions.getSubscriptions().get(i)
						.getHID());
			axisSubscription.setURL(subscriptions.getSubscriptions().get(i)
					.getURL().toString());
	...
	public boolean clearSubscriptions(String subscriber)
			throws java.rmi.RemoteException {
		try {
			Iterator<String> topicsToRemoveIterator = subscriptions.clear(
					new URL(subscriber)).iterator();
	...
	public boolean clearSubscriptionsWithHID(java.lang.String hid)
		...
		try {
			if (!Boolean.parseBoolean(configurator
					.get(EventManagerConfigurator.USE_NETWORK_MANAGER)))
				return false;
			Iterator<String> topicsToRemoveIterator = subscriptions.clear(hid)
					.iterator();
			while (topicsToRemoveIterator.hasNext()) {
				String topic = topicsToRemoveIterator.next();
				topics.remove(new Topic(topic, getTopicPriority(topic)));
	...
	@Override
	public boolean publish(final java.lang.String topicPattern,
			final eu.linksmart.eventmanager.Part[] event)
			throws java.rmi.RemoteException {
		log.info("EventManager publish on: " + topicPattern);
		Thread t = new Thread(new Runnable() {
			public void run() {
				for (final LocalSubscription subscription : subscriptions
						.getMatches(topicPattern)) {
					new Thread(new Runnable() {
						...
						public void run() {
							...
							if (subscription.getCounter() < MAXRETRIES) {
								...
								try {
									subscription._getPort().notify(
											topicPattern, event);
									// TODO!!!
								} catch (RemoteException e) {
									subscription.increaseCounter();
									log.error("No anwser from subscriber..."
											+ e.getMessage(), e);
								}
							} else {
		...
		if (topicPriority == -1)
			return true;
		threads.runTask(new Publication(t, new Topic(topicPattern,
				topicPriority)));
	...
	public boolean setPriority(java.lang.String topic, int priority) {
		// set the priority of the topic to the given value, order the topics
		// structure and the threads structure
		if (priority <= 0)
			return false;
		Iterator<Topic> topicsIterator = topics.iterator();
		while (topicsIterator.hasNext()) {
