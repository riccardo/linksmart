...
namespace OpenSource.UPnP
{
    ...
    /// </summary>
    /// 
    public class UPnPDevice //Removed sealed Peeter Kool Hydra project
    {
        ...
        public struct InvokerInfoStruct //Changed from Internal to Public by Peeter Kool Hydra project
        {
        ...
        public Hashtable InvokerInfo = Hashtable.Synchronized(new Hashtable());//Changed from Internal to Public by Peeter Kool Hydra project
        ...
        //Added by Peeter Kool Hydra project
        XmlDocument m_remotedevicexml = null;
        ...
        public virtual void AddCustomFieldInDescription(string FieldName, string FieldValue, string Namespace) //Added Virtual Peeter Kool Hydra project
        {
        ...
        public virtual string GetCustomFieldFromDescription(string FieldName, string Namespace) //Added Virtual Peeter Kool Hydra project
        {
        ...
        public System.Drawing.Image Icon
        {
            ...
            set
            {
                if (this.ControlPointOnly == false)
                {
                    _icon = value;
                }
            }
        ...
        {
            UPnPDevice d = new UPnPDevice(DeviceExpiration, version, RootDir);
            return (d);
        }
        /// <summary>
        ...
        {
            UPnPDevice d = new UPnPDevice(version, UDN);
            return (d);
        }
        ...
        /// </summary>
        public UPnPDevice() //Changed to public from internal Peeter Kool Hydra project
        {
        ...
        public UPnPDevice(double version, String UDN) //Changed to public from internal Peeter Kool Hydra project
        {
            OpenSource.Utilities.InstanceTracker.Add(this);
            // Full Device
            IsRoot = false;
            ...
            else
            {
                ...
                TempNum.ATTRMARK = ".";
                TempNum[0] = version.ToString().Replace(',','.');
        ...
        public UPnPDevice(int DeviceExpiration, double version, String RootDir) //Changed to public from internal Peeter Kool Hydra project
        {
            OpenSource.Utilities.InstanceTracker.Add(this);
            // Full Device
            IsRoot = true;
        ...
        private void UpdateDeviceSink2(UPnPDevice d)
        {
            foreach (UPnPService s in d.Services)
            {
                ...
                else
                {
                     ...
                     *	Device Vendor fscked up if this section 
                     *	executes
                     * 
                     */
                }
        ...
        public void UpdateDevice(Uri LocationUri, IPAddress HostInterface)
        {
            lock (UpdateTable)
            {
                UPnPDeviceFactory df = new UPnPDeviceFactory(LocationUri, 250, new UPnPDeviceFactory.UPnPDeviceHandler(UpdateDeviceSink), null, null);
                UpdateTable[df] = df;
            }
        }
       ...
       public virtual HTTPMessage Post(String MethodData, String XML, String SOAPACTION, HTTPSession WebSession)
        {
            return (Invoke(MethodData.Substring(1), XML, SOAPACTION, WebSession));
        }


        public virtual HTTPMessage Get(String GetWhat, IPEndPoint local) //Changed from private to public virtual by Peeter Kool Hydra project
        {
        ...
        private void HandleSearch(String ST, IPEndPoint src, IPEndPoint local)
        {
            ...
            if (local.AddressFamily == AddressFamily.InterNetwork)
            {
                ...
                {
                    System.Console.WriteLine("IoT added catch in UPnPDevice:" + e.Message);
                }
            }
        ...
        public virtual HTTPMessage Invoke(String Control, String XML, String SOAPACTION, HTTPSession WebSession)//Changed from private to public virtual by Peeter Kool Hydra project    
        {
            String MethodTag = "";
        ...
        public String StandardDeviceType
        {
            ...
            set
            {
                DeviceURN = "urn:schemas-upnp-org:device:" + value + ":" + Version;
            }
        }
        ...
        static public void ParseDevice(String XML, ref UPnPDevice RetVal) //Changed to public from private by Peeter Kool Hydra project
        {
            string TempString;
            UPnPService service;
            DText p = new DText();
                ...
                while (XMLDoc.LocalName != "device" && !XMLDoc.EOF)
                {
                    switch (XMLDoc.LocalName)
                    {
                        ...
                        case "modelNumber":
                            ...
                            break;
                        case "modelURL":
                            try { RetVal.ModelURL = new Uri(XMLDoc.ReadString()); } catch { }
                            break;
                        case "serialNumber":
        ...
        //IoT added functions by Peeter Kool
        public void AddIoTAnnotations()
        {
            XmlDocument myDoc = new XmlDocument();

            try
            {
                myDoc.Load(this.ManufacturerURL);

                XmlNodeList myNodes = myDoc.SelectNodes(".//*[@*[name()='IoTannotation']]");

                int upperbound, pos;

                upperbound = myNodes.Count;

                for (pos = 0; pos < upperbound; pos++)
                {
                    string name, url;

                    url = myNodes[pos].Attributes.GetNamedItem("IoTannotation").InnerText;
                    name = myNodes[pos].SelectSingleNode("*[name()='name']").InnerText;

                    this.AddCustomFieldInDescription(name, url, "IoT");
            ...
            {
                AddCustomFieldInDescription("errormessage", e.Message, "IoT");
            }

        }

        public void RefreshDeviceXml()
        {
            if (m_remotedevicexml == null)
                m_remotedevicexml = new XmlDocument();

            try
            {

                m_remotedevicexml.Load(LocationURL);
        ...
        public XmlDocument GetCurrentDeviceXml()
        {
            return m_remotedevicexml;
        }

        public void LoadDeviceXml(string xml)
        {
            try
            {
                if (m_remotedevicexml == null)
                ...
                m_remotedevicexml.LoadXml(xml);
            }

            catch (Exception e)
            {
                AddCustomFieldInDescription("errormessage", e.Message, "IoT");
                m_remotedevicexml = null;
            }
        }
