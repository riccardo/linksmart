...
namespace OpenSource.UPnP
{
    ...
    /// </summary>
    public  class UPnPSmartControlPoint //Removed sealed Peeter Kool Hydra project
    {
        ...
        virtual public void HandleAddedDevice(UPnPInternalSmartControlPoint sender, UPnPDevice device)//Changed from private by Peeter Kool Hydra project
        {
        ...
        virtual public void HandleUpdatedDevice(UPnPInternalSmartControlPoint sender, UPnPDevice device) //Changed from private by Peeter Kool Hydra project
        {
        ...
        virtual public void HandleRemovedDevice(UPnPInternalSmartControlPoint sender, UPnPDevice device) //Changed from private by Peeter Kool Hydra project
        {
            if ((OnRemovedDevice != null) || (OnRemovedService != null))
            {
                ...
                for (int filterIndex = 0; filterIndex < PartialMatchFilters.Length; ++filterIndex)
                {
                    ...
                    else
                    {
                        foreach (object x in r)
                        {
                            if ((x.GetType().FullName == "OpenSource.UPnP.UPnPDevice") || (x.GetType().FullName == "Intel.UPNP.UPnPDevice") || (x.GetType().FullName == "IoT.IoTDevice"))//Added (x.GetType().FullName == "Intel.UPNP.UPnPDevice") || (x.GetType().FullName == "IoT.IoTDevice") by Peeter Kool Hydra project)
                            {
                                dList.Add((UPnPDevice)x);
                                if (PartialMatchFilters.Length == 1)
                                {
