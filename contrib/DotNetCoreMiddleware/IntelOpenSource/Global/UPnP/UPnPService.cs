...
namespace OpenSource.UPnP
{
    ...
    public class UPnPAction : ICloneable
    {
        ...
        internal void GetXML(XmlTextWriter XDoc)
        {
            ...
            XDoc.WriteAttributeString("IoTannotation", this.ParentService.ParentDevice.GetCustomFieldFromDescription(this.Name, "IoT"));
    ...
    public sealed class UPnPService : ICloneable
    {
        ...
        //IoT added members by Peeter Kool
        private string m_soaptunnelIPaddress = "127.0.0.1";
        private string m_soaptunnelprefix = "/SOAPTunneling/0/";
        private string m_soaptunnelsuffix = "/0/hola";
        private int m_soaptunnelport = 8082;

        //IoT added method by Peeter Kool
        ...
        public String SCPDURL;//Changed to public from internal Peeter Kool Hydra project
        public String ControlURL //Changed to public from internal Peeter Kool Hydra project
        {
            get
            {
        ...
        public String EventURL //Changed  to public from internal by Peeter Kool Hydra project
        {
            get
            {
        ...
        public string __controlurl; //Changed from internal to public Peeter Kool Hydra project
        public string __eventurl; //Changed from internal to public Peeter Kool Hydra project
        public string SCPDFile //Changed from internal to public Peeter Kool Hydra project
        {
            get
            {
            ...
            : this(version)
        {
            ...
            SCPDURL = "_" + ServiceID + "_scpd.xml";
            ControlURL = "_" + ServiceID + "_control";
            EventURL = "_" + ServiceID + "_event";
        }
        ...
        public UPnPService(double version, Object InstanceObject)
            : this(version)
        {
        ...
        public void ParseSCPD(String XML) //Changed to public from internal by Peeter Kool Hydra project
        {
            ...
            UPnPService.UPnPServiceInvokeErrorHandler ErrorCallback)
        {
            ...
            //Whole try block is added IoT code by Peeter Kool
            try
            {
                string serviceid = null;

                if (this.ParentDevice.ParentDevice != null)
                    serviceid = ParentDevice.ParentDevice.GetCustomFieldFromDescription("IoTidUPnPService_" + this.ServiceID.Replace(":", "_"), "IoT");
                else
                    serviceid = ParentDevice.GetCustomFieldFromDescription("IoTidUPnPService_" + this.ServiceID.Replace(":", "_"), "IoT");



                if (serviceid != null && !this.ParentDevice.DeviceURN.Contains("discoverymanager"))
                {

                    sName = m_soaptunnelprefix + serviceid + m_soaptunnelsuffix;
                    WebIP = m_soaptunnelIPaddress;
                    WebPort = m_soaptunnelport;

                }
            }
            catch (Exception e)
            {
                throw new UPnPInvokeException(MethodName, InVarArr, "IoT special1: " + e.Message);
            }
            ...
            {
                throw new UPnPInvokeException(MethodName, InVarArr, "IoT special2: " + e.Message);
            }
        }
        /// <summary>
        ...
        //Added IoTInvokeLocal by Peeter Kool Hydra project
        public object IoTInvokeLocal(string MethodName, ref ArrayList VarList)
        {
            return InvokeLocal(MethodName, ref VarList);
        }

        public object InvokeLocal(String MethodName, ref ArrayList VarList) //Changed from Internal to Public by Peeter Kool Hydra project
        {
        ...
        public void AddStateVariable(UPnPStateVariable NewVar)
        {
            ...
            for (int x = 0; x < Nodes.Length; ++x)
            {
                ...
                if (A != null) A.GetArg(Nodes[x].ArgName).StateVarName = NewVar.Name;
            }
        ...
        public UPnPStateVariable GetStateVariableObject(String MethodName, String ArgName)
        {
            ...
            foreach (UPnPStateVariable SV in x)
            {
                ...
                { 
                    if ((n.ActionName == MethodName) && (n.ArgName.ToLower()== ArgName.ToLower())) //Case insentivity added py Peeter Kool Hydra project
                    {
                        RetVal = SV;
                        break;
                    }
